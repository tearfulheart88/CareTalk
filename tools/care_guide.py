...
_FAQ: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "answer": "현재 프로토타입은 사용자가 입력한 안부·건강 기록을 로컬 저장소에 보관합니다. 실제 운영 전에는 당사자 동의, 가족·복지사별 접근권한, 암호화와 삭제 기한이 반드시 필요합니다.",
...
def build_care_guide(
    ...
    if action == "start" and audience == "senior":
        ...
        replies = ["오늘은 괜찮아요", "밥 먹었어요", "조금 아파요", "도움이 필요해요", "사용법 보기"]
        cards = [
            _card(
                "한 번만 눌러도 안부 완료",
        ...
        replies = ["안전계획 만들기", "오늘 안부 보기", "가족 리포트", "개인정보 원칙", "FAQ"]
        cards = [
            _card(
                "동의부터 시작하는 돌봄",
        ...
        message = "실제 이름, 전화번호, 상세 주소, 진료기록은 공개 데모에 입력하지 마세요. 필요한 정보와 동의 범위를 먼저 정합니다."
    ...
    return {
        ...
        "audience": audience,
        "message": message,
        "purpose": "어르신의 짧은 선택을 가족·복지사의 실제 확인으로 연결합니다.",
